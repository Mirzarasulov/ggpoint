...
export function MobileNav() {
  ...
  // Блокировать скролл body + Escape для закрытия.
  useEffect(() => {
    if (!open) return;
  ...
  // Panel рендерим через portal в <body>, чтобы вырваться из containing block,
  // создаваемого `backdrop-filter` у Header.
  const panel = (
    <div
  ...
  return (
    <>
      <button
        ...
        className="md:hidden inline-flex h-11 w-11 items-center justify-center rounded-lg text-foreground hover:bg-surface transition-colors"
      >
